...
int main( int argc, char** argv ){
    const std::vector<std::string> day{ "Monday", "Tuesday", "Wednesday", "Thursday", "Friday" };
    const int POS_SATURDAY = 5;

    std::string s;
    cin >> s;

    int i = 0;
    for( i = 0; i < static_cast<int>(day.size()); i++ ){
        if( day[i].compare( s ) == 0 ) break;
    }

    cout << (POS_SATURDAY + 1) - (i + 1) << endl;
